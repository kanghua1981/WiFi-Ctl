...
/*
    ...
    {
        sprintf(cmd,"uci set wireless.wifi0.channel=auto");
        system(cmd);
    }
    else if (ssidinfo->channel > 0 && ssidinfo->channel < 14 )
    {
        sprintf(cmd,"uci set wireless.wifi0.channel=%d",ssidinfo->channel);
        system(cmd);
    }
    else
    {
...
int getWiFiCountry(char *country)
{
    FILE *pp = NULL;
    char cmd[64] = {0};
    char buffer[8] = {0};
    int channel = 1;
    sprintf(cmd,"uci get wireless.wifi0.country");
    pp = popen(cmd,"r");
    if (pp == NULL)
    {
        return -1;
...
void syswps()
{
	system("hostapd_cli -p /var/run/hostapd-wifi0 -i SSID1 wps_pbc 1>/dev/NULL");
}
...
void reload_network()
{
    system("/sbin/reloadnetwork.sh simplereload");
    system("sleep 1");

    system("/etc/init.d/fireware restart");
}
                        ...
                        char *strDNS1,char *strDNS2)
{
    ...
    system(cmd);
	sprintf(cmd,"uci set network.lan.ifname=eth1");
    system(cmd);
...
{
	char cmd[64] = {0};
	if (strcmp(domain,"lan")!=0)
		return -1;
	
    sprintf(cmd,"uci delete network.wan");;
    system(cmd);
	sprintf(cmd,"uci set network.lan.ifname='eth1 eth0'");;
	system(cmd);
	system("uci commit network");
	setConnectMode(CONNECT_WIRED_MODE);
	return 0;
...
int set_wifi_basic(char *channel,char *strenth,char *strCountry)
{
    ...
    if (strcmp(channel,"0") == 0)
        ...
        sprintf(cmd,"uci set wireless.wifi0.channel=%s",channel);

	
    system(cmd);

	sprintf(cmd,"uci set wireless.wifi0.country=%s",strCountry);
	system(cmd);
    system("uci commit wireless");
    //system("/sbin/reloadnetwork.sh wifi");
    return 0;
...
int get_wifi_basic(char *channel,char *strStrenth,char *strCountry)
{
    int ichn;
    ichn = getWiFiChannel("SSID1");
    sprintf(channel,"%d",ichn);
    sprintf(strStrenth,"100",strStrenth);
	getWiFiCountry(strCountry);
    return 0;
}
...
int getWiFiEncryption(char *ifname,char *encry)
{
    ...
    char cmd[128] = {0};
    char buffer[32] = {0};
    sprintf(cmd,"getssidencry.sh %s 2>/dev/null",ifname);
    pp = popen(cmd,"r");
    if (pp == NULL)
    {
        return -1;
...
int apply_static_dhcp()
{
    char buff[128];
    FILE *fp = NULL;
    sprintf(buff,"/sbin/static_dhcp.sh apply");
    fp = popen(buff,"r");
    if (fp == NULL)
    {
        printf("exec static_dhcp apply error");
        return -1;
    }
    pclose(fp);
    return 0;
...
int add_static_dhcp(char *mac,char *ip)
{
    char buff[128];
    FILE *fp = NULL;
    sprintf(buff,"/sbin/static_dhcp.sh add %s %s 2>/dev/null",mac,ip);
    fp = popen(buff,"r");
    if (fp == NULL)
    {
        printf("exec static_dhcp error");
        return -1;
    }
    printf("exec ok\n");
    pclose(fp);
...
int del_static_dhcp(char *mac,char *ip)
{
    char buff[128];
    FILE *fp = NULL;
    sprintf(buff,"/sbin/static_dhcp.sh del %s %s 2>/dev/null",mac,ip);
    printf("%s\n",buff);
    fp = popen(buff,"r");
    if (fp == NULL)
    {
...
int modify_static_dhcp(char *mac,char *ip)
{
    char buff[128];
    FILE *fp = NULL;
    sprintf(buff,"/sbin/static_dhcp.sh modify %s %s 2>/dev/null",mac,ip);
    printf("%s\n",buff);
    fp = popen(buff,"r");
    if (fp == NULL)
    {
...
int clear_static_dhcp()
{
    char buff[128];
    FILE *fp = NULL;
    sprintf(buff,"/sbin/static_dhcp.sh clear 2>/dev/null");
    fp = popen(buff,"r");
    if (fp == NULL)
    {
        printf("exec static_dhcp error");
...
int list_static_dhcp(STATIC_DHCP_TABLE *dhcptable)
{
    ...
    sprintf(cmd,"/sbin/static_dhcp.sh list");
    pp = popen(cmd,"r");
    if (pp == NULL)
        return -1;
	//printf("getWlanStaInfo %s\r\n",ifname);
    while (feof(pp) == 0)
    {
    	memset(buffer,0,256);
        fgets(buffer,256,pp);
        ...
        if (strlen(buffer) > 0)
        {
            sscanf(buffer,"%s %s",dhcptable->entry[i].mac,dhcptable->entry[i].ip);
			//printf("%s %s\r\n",mactable->entry[i].mac,mactable->entry[i].rssi);
            i++;
        }
    }
	pclose(pp);
...
int getWlanStaInfo(char *ifname,STA_MAC_TABLE *mactable)
{
    ...
    while (feof(pp) == 0)
    {
        ...
        if (strlen(buffer) > 0)
        {
            sscanf(buffer,"%*s %s %s %*s %*s %s %s",mactable->entry[i].mac,
                    mactable->entry[i].ip,
                    mactable->entry[i].rssi,
                    mactable->entry[i].rxrate);
			//printf("%s %s\r\n",mactable->entry[i].mac,mactable->entry[i].rssi);
...
int get_wan_broadcast(char *bcast_addr)
{
    FILE *fp = NULL;
    char buff[32] = {0};
    fp = popen("/sbin/getwanbroadcast.sh","r");
    if (fp == NULL)
        return -1;
    fgets(buff,32,fp);
    if (strlen(buff)==0)
        return -1;
    sscanf(buff,"Bcast:%s",bcast_addr);
    pclose(fp);
    return 0;
}

int broadcast_thread(void *param)
{
   int fd;
   ...
   ret = get_wan_broadcast(bcast_addr);
   if (ret != 0)
       return -1;
   ...
   addrto.sin_port=htons(9992); 
   //addrto.sin_addr.s_addr = inet_addr("255.255.255.255"); 
   addrto.sin_addr.s_addr = inet_addr(bcast_addr); 
   setsockopt(fd, SOL_SOCKET, SO_BROADCAST, &opt, sizeof(opt)); 
   get_ssid(0,ssid);
   get_ssid_password(0,password);
   get_ssid_authmode(0,auth);
   sprintf(sendbuf,"ssid=%s,auth=%s,pwd=%s",ssid,auth,password);
   printf("ssid=%s,auth=%s,pwd=%s timeout =%d",ssid,auth,password,timeout);

   while(timeout-- > 0)
   {
       ret = sendto(fd,sendbuf,strlen(sendbuf),0,(struct sockaddr *)&addrto,sizeof(struct sockaddr_in)); 
       if (ret <= 0)
...
int broadcast_wirelesscode(int timeout)
{
#if 0
    pthread_t id_1;
    int ret = 0;
    ret = pthread_create(&id_1,NULL,(void *) broadcast_thread,timeout);  
    if(ret!=0)  
    {  
...
int setWirelessReapter(int status,char *ssid,char *bssid,char *channel,char *auth,char *encry,char *key)
{
    ...
    sprintf(cmd,"/sbin/uci set wireless.wifi0.channel=%s",channel);
    system(cmd);

    sprintf(cmd,"/sbin/uci set wireless.SSID4=wifi-iface");
    system(cmd);
    sprintf(cmd,"/sbin/uci set wireless.SSID4.device=wifi0");
    system(cmd);
...
int  systemupgrade(char *imagename,char *md5,char *version,char *needclear)
{
    ...
    if (-1 == access("/tmp/upgradefirmware.bin",0))
        return -1;

    sprintf(cmd, "/sbin/upgrade_raw.sh upgradefirmware.bin %s %s &",version, needclear);
    system(cmd);
    return 0;
}
